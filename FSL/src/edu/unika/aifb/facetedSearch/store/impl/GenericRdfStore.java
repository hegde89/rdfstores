...
public class GenericRdfStore implements IStore {
	...
	private void createStore(Properties props) throws IOException,
		...
		String files = props
				.getProperty(FacetedSearchLayerConfig.Property.FILES);
		...
		Importer importer;
		if (props.getProperty(FacetedSearchLayerConfig.Property.ONTO_LANGUAGE)
				.equals(FacetEnvironment.OntologyLanguage.N_3)) {
			importer = new NxImporter();
		} else if (props.getProperty(
				FacetedSearchLayerConfig.Property.ONTO_LANGUAGE).equals(
			...
			throw new InvalidParameterException(ExceptionHelper.createMessage(
					FacetedSearchLayerConfig.Property.FILES,
		...
		IndexCreator ic = new IndexCreator(
				m_idxDir = new IndexDirectory(
						props
								.getProperty(FacetedSearchLayerConfig.Property.GRAPH_INDEX_DIR)));

		importer
				.setIgnoreDataTypes(new Boolean(
						props
								.getProperty(FacetedSearchLayerConfig.Property.IGNORE_DATATYPES)));
		...
		ic
				.setCreateDataIndex(new Boolean(
						props
								.getProperty(FacetedSearchLayerConfig.Property.CREATE_DATA_INDEX)));

		// create structure index (default: true)
		ic
				.setCreateStructureIndex(new Boolean(
						props
								.getProperty(FacetedSearchLayerConfig.Property.CREATE_STRUCTURE_INDEX)));

		// create keyword index (default: true)
		ic
				.setCreateKeywordIndex(new Boolean(
						props
								.getProperty(FacetedSearchLayerConfig.Property.CREATE_KEYWORD_INDEX)));

		// set neighborhood size to 2 (default: 0)
		ic
				.setKWNeighborhoodSize(Integer
						.parseInt(props
								.getProperty(FacetedSearchLayerConfig.Property.NEIGHBORHOOD_SIZE)));

		// set structure index path length to 1 (default: 1)
		ic
				.setSIPathLength(Integer
						.parseInt(props
								.getProperty(FacetedSearchLayerConfig.Property.STRUCTURE_INDEX_PATH_LENGTH)));

		// include data values in structure index (not graph) (default: true)
		ic
				.setStructureBasedDataPartitioning(new Boolean(
						props
								.getProperty(FacetedSearchLayerConfig.Property.STRUCTURE_BASED_DATA_PARTIONING)));

		ic
				.setSICreateDataExtensions(new Boolean(
						props
								.getProperty(FacetedSearchLayerConfig.Property.CREATE_DATA_EXTENSIONS)));
	...
	public GenericQueryEvaluator getEvaluator(SearchSession session) {
		...
		if (m_evalPool.get(session.getSearchSessionId()) == null) {
			m_evalPool.put(session.getSearchSessionId(),
					new GenericQueryEvaluator(session, m_idxReader));
		}
