...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		op.accepts("o", "output directory").withRequiredArg().ofType(
				String.class).describedAs("directory");
		op.accepts("sk", "structure index path length").withRequiredArg()
				.ofType(Integer.class).describedAs(
						"structure index path length, default: 0");
		op.accepts("nk", "neighborhood size").withRequiredArg().ofType(
				Integer.class).describedAs("neighborhood size, default: 0");
		op.accepts("kw", "keyword index");
		op.accepts("sd", "data extensions");
		op.accepts("resume", "resume from analyzse, structure or keyword")
		...
		boolean tsranking = os.has("tsrank");

		if (tsranking) {
			log.debug("Using ts ranking");
		}
		...
		ic.setOption(IndexConfiguration.TSRANKING, tsranking);
		...
		// ic.setOption(IndexConfiguration.SP_ELIMINATE_REFLEXIVE_EDGES,
		// eliminateRE);
