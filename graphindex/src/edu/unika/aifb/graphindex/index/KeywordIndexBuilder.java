...
public class KeywordIndexBuilder {
	...
	private static final Logger log = Logger
			.getLogger(KeywordIndexBuilder.class);

	public KeywordIndexBuilder(
			edu.unika.aifb.graphindex.index.IndexReader idxReader,
			boolean resume) throws IOException, StorageException {
	...
	public void indexKeywords() throws StorageException, IOException {
		File indexDir = idxDirectory.getDirectory(IndexDirectory.KEYWORD_DIR,
				!resume);
		File valueDir = idxDirectory.getDirectory(IndexDirectory.VALUE_DIR,
				!resume);

		this.objectProperties = Util.readEdgeSet(idxDirectory
				.getFile(IndexDirectory.OBJECT_PROPERTIES_FILE));
		this.relations = Util.readEdgeSet(idxDirectory.getTempFile("relations",
				false));
		this.attributes = Util.readEdgeSet(idxDirectory.getTempFile(
				"attributes", false));
		properties = new HashSet<String>();
		properties.addAll(relations);
		properties.addAll(attributes);

		log.debug("attributes: " + attributes.size() + ", relations: "
				+ relations.size());

		try {
			// HyphenationCompoundWordAnalyzer analyzer = new
			// HyphenationCompoundWordAnalyzer("./res/en_hyph_US.xml",
			// "./res/en_US.dic");
			// DictionaryCompoundWordAnalyzer analyzer = new
			// DictionaryCompoundWordAnalyzer("./res/en_US.dic");
			...
			if (resume) {
				reader = org.apache.lucene.index.IndexReader.open(FSDirectory
						.getDirectory(indexDir), true);
				log.debug("docs: " + reader.numDocs());
			}

			if (!resume) {
				log.info("Indexing concepts");
				indexSchema(indexWriter, idxDirectory.getTempFile("concepts",
						false), TypeUtil.CONCEPT, CONCEPT_BOOST);

				log.info("Indexing attributes");
				indexSchema(indexWriter, idxDirectory.getTempFile("attributes",
						false), TypeUtil.ATTRIBUTE, ATTRIBUTE_BOOST);

				log.info("Indexing relations");
				indexSchema(indexWriter, idxDirectory.getTempFile("relations",
						false), TypeUtil.RELATION, RELATION_BOOST);
			}
	...
	protected void indexSchema(IndexWriter indexWriter, File file, String type,
			float boost) throws StorageException {
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				doc.add(new Field(Constant.TYPE_FIELD, type, Field.Store.YES,
						Field.Index.NOT_ANALYZED_NO_NORMS));

				// indexing local name
				doc.add(new Field(Constant.SCHEMA_FIELD, localName,
						Field.Store.YES, Field.Index.ANALYZED,
						Field.TermVector.NO));

				// indexing label
				Table<String> table = dataIndex.getTriples(uri, RDFS.LABEL
						.stringValue(), null);
				for (String[] row : table) {
					doc.add(new Field(Constant.SCHEMA_FIELD, row[2],
							Field.Store.YES, Field.Index.ANALYZED,
							Field.TermVector.NO));
				}

				// indexing uri
				doc.add(new Field(Constant.URI_FIELD, uri, Field.Store.YES,
						Field.Index.NOT_ANALYZED_NO_NORMS));

				// indexing extension id for concept
				if (type.equals(TypeUtil.CONCEPT)
						&& idxConfig.getBoolean(IndexConfiguration.HAS_SP)) {
					String blockName = blockSearcher.getBlockName(uri);
					doc.add(new Field(Constant.EXTENSION_FIELD, blockName,
	...
	private void indexEntity(IndexWriter indexWriter, File file,
			IndexReader reader) throws IOException, StorageException {
		try {
			BufferedReader br = new BufferedReader(new FileReader(file));
			String line;
			int entities = 0;
	...
	private Set<String> computeConcepts(String entityUri) throws IOException,
			StorageException {
		Set<String> set = new HashSet<String>();

		IndexDescription idx = dataIndex.getSuitableIndex(DataField.PROPERTY,
				DataField.SUBJECT);
		set = dataIndex.getIndexStorage(idx).getDataSet(
				idx,
				DataField.OBJECT,
				idx.createValueArray(DataField.PROPERTY,
						RDF.TYPE.stringValue(), DataField.SUBJECT, entityUri));

		return set;
	}

	private Set<String> computeLabels(String entityUri) throws IOException,
			StorageException {
		Set<String> set = new HashSet<String>();
		...
		set = dataIndex.getIndexStorage(idx).getDataSet(
				idx,
				DataField.OBJECT,
				idx.createValueArray(DataField.PROPERTY, RDFS.LABEL
						.stringValue(), DataField.SUBJECT, entityUri));
	...
	private Set<String> computeNeighbors(String entityUri) throws IOException,
			StorageException {
	...
	private Set<String> computeReachableEntities(String entityUri)
			throws IOException, StorageException {
