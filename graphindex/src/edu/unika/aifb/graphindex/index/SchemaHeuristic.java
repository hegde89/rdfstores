...
public class SchemaHeuristic {
	...
	private final static String[] conEdges = { RDF.BAG.stringValue(),
			RDF.SEQ.stringValue(), RDF.ALT.stringValue(),
			RDF.LIST.stringValue(), RDF.FIRST.stringValue(),
			RDF.REST.stringValue(), RDF.NIL.stringValue() };

	private final static String[] rdfsEdges = { RDF.TYPE.stringValue(),
			RDFS.SUBCLASSOF.stringValue(), RDFS.DOMAIN.stringValue(),
			RDFS.RANGE.stringValue(), RDFS.SUBPROPERTYOF.stringValue(),
			RDFS.LABEL.stringValue(), RDFS.COMMENT.stringValue(),
			RDFS.CLASS.stringValue(), OWL.CLASS.stringValue(),
			OWL.OBJECTPROPERTY.stringValue(),
			OWL.DATATYPEPROPERTY.stringValue(), RDFS.SEEALSO.stringValue(),
			RDFS.ISDEFINEDBY.stringValue(), OWL.ONTOLOGY.stringValue(),
			OWL.ANNOTATIONPROPERTY.stringValue(), RDFS.MEMBER.stringValue(),
			RDFS.CONTAINER.stringValue() };
	...
	public static String getSubjectType(String pred, String obj) {
		if ((pred.equals(RDF.TYPE.stringValue()) && (obj.equals(RDFS.CLASS
				.stringValue()) || obj.equals(OWL.CLASS.stringValue())))
				|| pred.equals(RDFS.SUBCLASSOF.stringValue()) || pred.contains("type")) {
			return CONCEPT;
		} else if ((pred.equals(RDF.TYPE.stringValue()) && (obj
				.equals(OWL.OBJECTPROPERTY.stringValue()) || obj
				.equals(OWL.DATATYPEPROPERTY.stringValue())))
	...
	public static String checkType(String predicate, String object) {
		...
		if (TypeUtil.getSubjectType(predicate, object).equals(TypeUtil.ENTITY)
				&& TypeUtil.getObjectType(predicate, object).equals(
						TypeUtil.ENTITY)
				&& TypeUtil.getPredicateType(predicate, object).equals(
						TypeUtil.RELATION)) {
			type = TypeUtil.RELATION;
		} else if (TypeUtil.getSubjectType(predicate, object).equals(
				TypeUtil.ENTITY)
				&& TypeUtil.getObjectType(predicate, object).equals(
						TypeUtil.LITERAL)
				&& TypeUtil.getPredicateType(predicate, object).equals(
