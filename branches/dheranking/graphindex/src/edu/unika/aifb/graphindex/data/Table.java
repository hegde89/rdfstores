...
public class Table<T extends Comparable<T>> implements Iterable<T[]>, Cloneable, Serializable {
	...
	public void subTable(int fromIndex, int toIndex) {
		m_rows = m_rows.subList(fromIndex, toIndex);
	}

	public void removeAllExcept(int fromIndex, int toIndex) {
		List<T[]> rows = new ArrayList<T[]>(toIndex - fromIndex + 1);
		for (int i = fromIndex; i < Math.min(toIndex, m_rows.size() - 1); i++)
			rows.add(m_rows.get(i));
		m_rows = rows;
	}
