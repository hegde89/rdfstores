...
<A HREF=http://www.mpi-inf.mpg.de/~suchanek/downloads/javatools target=_blank>
  ...
  Example:
  <PRE>
     normalize("This String contains the umlauts Ä, Ö and Ü");
  ...
  Example:
  <PRE>
     encodePercentage("This String contains the umlauts Ä, Ö and Ü");
...
public class Char {
  ...
  static {
    Object[] o=new Object[]{
        ...
        (char)8836, "NOT SUBSET OF",
        (char)8838, "SUBSET OR EQUAL",
        (char)8839, "SUPERSET OR EQUAL",
        (char)8853, "(+)",
        (char)8855, "(*)",
        (char)8869, "_|_",
        (char)8901, "*",
        ...
        (char)0xFB05,"ft",
        (char)0xFB06,"st"};
    for(int i=0;i<o.length;i+=2) normalizeMap.put((Character)o[i],(String)o[i+1]);
   } 
  ...
  public static char eatUtf8(String a, int[] n) {
    ...
    if(a.length()>=n[0]) { 
      switch(n[0]) {
        case 1: return(a.charAt(0));
        case 2: if((a.charAt(1)&0xC0)!=0x80) break;
                return((char)(((a.charAt(0)&0x1F)<<6)+(a.charAt(1)&0x3F)));
        case 3: if((a.charAt(1)&0xC0)!=0x80 ||
                   (a.charAt(2)&0xC0)!=0x80) break;
                return((char)(((a.charAt(0)&0x0F)<<12)
                             +((a.charAt(1)&0x3F)<<6)
                             +((a.charAt(2)&0x3F))));
        case 4: if((a.charAt(1)&0xC0)!=0x80 ||
                   (a.charAt(2)&0xC0)!=0x80 ||
                   (a.charAt(3)&0xC0)!=0x80) break;
  ...
  public static char eatBackslash(String a, int[] n) {
    ...
    if(a.startsWith("\\b")) { n[0]=2; return((char)8); }
    if(a.startsWith("\\t")) { n[0]=2; return((char)9); }
    if(a.startsWith("\\n")) { n[0]=2; return((char)10); }
    if(a.startsWith("\\f")) { n[0]=2; return((char)12); }
    if(a.startsWith("\\r")) { n[0]=2; return((char)13); }
    if(a.startsWith("\\\\")) { n[0]=2; return('\\'); }
    if(a.startsWith("\\\"")) { n[0]=2; return('"'); }
    if(a.startsWith("\\'")) { n[0]=2; return('\''); }
    // Octal codes
    n[0]=1;
  ...
  public static String encodePercentage(char c) {    
    ...
    if(c<'0' || c>'9' && c<'A') return("%"+Integer.toHexString(c).toUpperCase());    
    if(c<128) return(""+c);    
    String s=encodeUTF8(c);
    String result="";
    while(s.length()>0) {
      result+="%"+Integer.toHexString(s.charAt(0)).toUpperCase();
      s=s.substring(1);
    }
    return(result);
  }
  ...
  public static String normalize(String s) {
    StringBuilder b=new StringBuilder();
    for(int i=0;i < s.length();i++) b.append(normalize(s.charAt(i)));
    return(b.toString());
  }  
