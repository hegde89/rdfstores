...
Therefore, a HTTP URI reference can be seen as a data source, whose content can be obtained by
dereferencing the HTTP URI. Triples in these \emph{Linked Data sources} contain other HTTP URI references. Following these URIs lead to other sources. When it is clear from the context, we will make no distinction between the resource identified by an URI and the Linked Data source that can be retrieved by dereferencing that URI. \todo{do we really want this?}
...
% Since Linked Data triples in $T_D$ also form a graph, processing
...
to be \emph{retrieved via HTTP dereferencing during query processing}. \dtr{either motivate here why this is needed or in the intro}
