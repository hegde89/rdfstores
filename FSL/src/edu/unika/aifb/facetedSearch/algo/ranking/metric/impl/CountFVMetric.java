...
public class CountFVMetric implements IRankingMetric {
	...
	public CountFVMetric(SearchSession session) {
		m_session = session;
	}
	 ...
	 * (edu.unika.aifb.facetedSearch.facets.tree.model.impl.Node)
	 */
	public void computeScore(StaticClusterNode node) {
		node.setWeight(m_session.getCache().getCountFV(node));
	}
}
